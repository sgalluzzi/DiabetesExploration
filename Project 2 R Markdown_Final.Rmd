---
title: "Exploring Predictors Related to Diabetes"
author: "Seth Galluzzi (vzw6yk), Haley Egan (vkb6bn), JD Pinto (jp5ph), Sydney Masterson (bmp6tz)"
...
# Data Cleaning
Three columns were eliminated from the data.  The id column was eliminated because it has no intrinsic value.  The second diastolic and second systolic blood pressure columns were both eliminated because they had too many missing values. The median was used to replace the remaining columns.  The median was chosen instead of the mean because large values in the data were driving up the mean value. 
```{r}
df <- diabetes
...
The plots below include a histogram of glycosylated hemoglobin, a q-q plot of glycosylated hemoglobin, a scatter plot comparing glycosylated hemoglobin and stabilized glucose with gender, and several box plots.  All of the plots indicated a relationship between glycosylated hemoglobin and stabilized glucose.  The histogram also shows a positive skew.  After viewing the visualizations, a new categorical variable (diabetes) was created to identify how many subjects in our data set were a diabetes risk.  The data showed approximately 14% of the subjects in our data were a diabetes risk
```{r}
summary(df)

ggplot(df, aes(x=glyhb))+
  geom_histogram(na.rm=TRUE, fill = "#EF486D", binwidth = .5)+
  scale_x_continuous(n.breaks=20)+
  labs(x = "Glycosolated Hemoglobin", 
       title = "Glycosolated Hemoglobin Histogram")

qqnorm(df$glyhb, pch = 1, frame = FALSE)
qqline(df$glyhb, col="#EF486D")

plt_1 <- ggplot(df, aes(x=stab.glu ,y= glyhb, color=gender))+
  geom_point(shape=5)+
...
plt_1 + scale_color_manual(values = c("#359ED2", "#EF486D")) 
...
plt_2 <- (ggplot(df, aes(x=diabetes, fill = gender))+
  geom_bar()+
  labs(x = "Diabetes Risk", y = "Total Observations",
       title = "Diabetes Risk by Gender", fill = "Gender"))

plt_2 + scale_fill_manual(values = c("#359ED2", "#EF486D")) 
...
# Testing Process
The following code is used to test the accuracy of predicting diabetes risk.  A 5 predictor model and a 1 predictor model were tested during the process.  The ROC curves and confusion matrices are included.  The AUCs of the curves were nearly identical. 
```{r}
df <- diabetes
...
lines(x = c(0,1), y = c(0,1), col="#EF486D")
...
The following depicts the exploration completed during the proposal phase of this project.  
```{r}
library(faraway)
library(caret)
